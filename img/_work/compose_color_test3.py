...
OUT_DIR = r"C:\proyectos\Claude\zoo\img\cards_colortest3"
...
def build_palette(n):
    """Alterna morado/dorado, variando saturación y valor en cada carta
    para que cada una tenga un tono distinto dentro de esas dos familias
    (nunca tan claro que se pierda contraste sobre el fondo crema)."""
    palette = []
    half = (n + 1) // 2
    for i in range(n):
        is_purple = i % 2 == 0
        ...
        hue = PURPLE_HUE if is_purple else GOLD_HUE
        sat = 0.55 + 0.35 * t
        val = 0.32 + 0.30 * (1 - t)
