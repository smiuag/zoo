...
OUT_PDF = r"C:\proyectos\Claude\zoo\img\cartas_zoo_en.pdf"
...
def main():
    ...
    n_pages = pl.n_pages_for(grid, len(card_paths))
    ...
    print(
        f"\nSaved {OUT_PDF} ({n_pages} páginas de frente + 1 de reverso de "
        f"muestra, {len(card_paths)} cartas, {grid.cols}x{grid.rows} por página)"
    )
