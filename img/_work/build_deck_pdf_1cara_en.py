...
OUT_PDF = r"C:\proyectos\Claude\zoo\img\mazo_impresion_1cara_en.pdf"
...
# English print run: same copy counts as build_deck_pdf_1cara.py (single-
# sided, multiple of 9) — only the card art (Spanish vs English text) differs.
COUNTS = {
    "sloth": 20,
    "coin-1": 35,
    "coin-2": 17,
...
def main():
    ...
    if len(deck) % per_page != 0:
        raise SystemExit(
            f"El total ({len(deck)}) no es multiplo de {per_page}: ajusta COUNTS "
            "(quita/anade copias de coin-2 o coin-3) antes de generar el PDF."
        )
    ...
    print(f"\nSaved {OUT_PDF} ({n_pages} paginas a 1 cara, {len(deck)} cartas, multiplo de {per_page})")
    print("\nResumen de copias:")
    for cid in ordered_ids:
