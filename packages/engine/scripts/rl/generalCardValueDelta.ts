...
import { ANIMAL_SPECIES, applyAction, autoResolvePendingDiscard, createGame, getActivePlayer, type Action } from '../../src/engine';
...
const oldPath = fileURLToPath(new URL('../../src/bots/rl/weights.json', import.meta.url));
const newPath = fileURLToPath(new URL('../../src/bots/rl/weights-general-solo.json', import.meta.url));

const oldBot = createRlBot({ weights: loadWeights(oldPath) });
const newBot = createRlBot({ weights: loadWeights(newPath) });

const oldTally = playAndTally(oldBot, GAMES);
const newTally = playAndTally(newBot, GAMES);

const allSpecies = new Set<string>(ANIMAL_SPECIES);
...
console.log(
  `general-prod (weights.json) vs general-solo (weights-general-solo.json), ${GAMES} partidas cada uno, % de compras de animal por especie.\n`
);
...
const requested = ['hippopotamus', 'lion', 'elephant', 'turtle'];
console.log('\nEspecies pedidas (hipopótamo, león, elefante, tortuga):');
for (const species of requested) {
  const r = rows.find((row) => row.species === species);
  if (!r) {
    console.log(`  ${species.padEnd(15)} sin compras en ninguna de las dos tandas`);
    continue;
  }
  console.log(`  ${r.species.padEnd(15)} ${r.oldFreq.toFixed(1)}% -> ${r.newFreq.toFixed(1)}%  (${r.delta >= 0 ? '+' : ''}${r.delta.toFixed(1)} pp)`);
}

console.log('\nTabla completa, todas las especies (ordenada alfabéticamente):');
for (const r of [...rows].sort((a, b) => a.species.localeCompare(b.species))) {
  console.log(`  ${r.species.padEnd(15)} ${r.oldFreq.toFixed(1)}% -> ${r.newFreq.toFixed(1)}%  (${r.delta >= 0 ? '+' : ''}${r.delta.toFixed(1)} pp)`);
}
