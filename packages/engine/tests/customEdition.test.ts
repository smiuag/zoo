...
describe('edición "Personalizado" (edition: "custom")', () => {
  ...
  it('initialMarketCopies aplica el delta correcto por tramo de coste, con suelo de 1 copia', () => {
    // 2 jugadores, carta barata (coste<5): base 2+2=4.
    expect(initialMarketCopies(3, 2, { cheap: 1, expensive: 0 })).toBe(5);
    expect(initialMarketCopies(3, 2, { cheap: -2, expensive: 0 })).toBe(2);
    // 2 jugadores, carta cara (coste>=5): base 2. Un delta -2 se clampa a 1,
    // nunca 0 (una especie elegida nunca debe quedarse sin ninguna copia).
    expect(initialMarketCopies(7, 2, { cheap: 0, expensive: -2 })).toBe(1);
    expect(initialMarketCopies(7, 2, { cheap: 0, expensive: 2 })).toBe(4);
    // Sin deltas, se comporta exactamente igual que siempre.
